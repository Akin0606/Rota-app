...
import { ApiError, PinAuthData, authenticatePin, submitAvailability } from "@/lib/api";
...
export default function StaffAvailabilityPage({ params }: { params: { venue_token: string } }) {
  ...
  async function handleSubmit() {
    ...
    setSubmitting(true);
    try {
      await submitAvailability(venue_token, pin, submissions);
  ...
  if (!data.period) {
    return (
      <CenteredMessage>
        There&apos;s no availability window open for {data.venue_name} right now. Check back soon.
      </CenteredMessage>
    );
  }
        ...
        <div className="px-6 pb-7 pt-5">
          ...
          <div className="mb-5 flex justify-center">
            <div className="flex items-center gap-1.5 rounded-full bg-warn-bg px-3.5 py-1.5 text-xs font-semibold text-warn-text">
              ...
              Submit by{" "}
              {formatDeadline(period.week_start, data.rules.avail_closes_day, data.rules.avail_closes_time)}
            </div>
          </div>
